...
import firewallDataArray from "./Data-Container";

interface ErrorsContainerProps {
  selectedId: string; // или string, если id — строка
}

export default function ErrorsContainer({ selectedId }: ErrorsContainerProps) {
  const [showCode, setShowCode] = useState(false);
  const firewallItem = firewallDataArray.find(
    (item) => item.id === selectedId.toString(),
  );

  if (!firewallItem) {
    return (
      <div className="text-center text-red-500">
        Error: No data found for selected firewall issue.
      </div>
    );
  }
        ...
        <div>
          ...
          <ul className="space-y-3 mb-6">
            {firewallItem.solutions.map((solution, idx) => (
              <li key={idx} className="flex items-start">
                <svg
                  className="w-4 h-4 text-success-500 mr-2 mt-0.5"
                  fill="none"
                  stroke="currentColor"
                  strokeWidth="2"
                  viewBox="0 0 24 24"
